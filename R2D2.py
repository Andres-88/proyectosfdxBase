...
def createBigXMLs(start=0, end=30,xmlVersion=1):
    ...
    if not f in not_necessary:
        for j in range(start,end):
            ...
            metadata.text = members[j]['fullName']
    ...
    tree.write('packages/package'+name.text+str(xmlVersion)+'.xml', xml_declaration=True, encoding='utf-8')
    if end >= len(members):
        return
    else:
        createBigXMLs(end,end+30,xmlVersion+1)
